...
- Activity reads the shared customer click rows in `src/data/reporting.mock.ts`. Every dealership has at least 10 customers with click stats. Opening a dealership fixes that dealership as the activity scope. The title bar has a **Back** button that returns to `/reports`.
