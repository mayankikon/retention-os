...
Creators can reopen a **Draft** campaign from detail, change setup fields in the existing New Campaign wizard, Save Draft without losing the campaign identity, and either return to Draft detail or continue toward Activate from Review.
...
- No Activate button on detail itself (activation stays on Review).
...
- Status stays `draft` until Activate from Review succeeds.
...
- Same **Activate** + **Save Draft** UI as create Review.
- Activate moves the campaign to Active. A future configured start date gates sends without introducing a Scheduled status.
